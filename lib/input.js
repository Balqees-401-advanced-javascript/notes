...
class Input{
  constructor(){
    const args = minimist(process.argv.slice(2)); // give me all the argument (obj of command line) (take user command) //trans the array to obj
  ...
  validation(action){
    let validMethod =/^--add$|^-a$|^--list$|^--delete$/;
